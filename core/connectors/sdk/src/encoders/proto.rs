...
impl ProtoStreamEncoder {
    ...
    fn encode_with_message_descriptor(
        ...
        let json_value = match payload {
            ...
            Payload::Bson(bson) => simd_json::to_owned_value(&mut bson::serialize_to_vec(&bson).map_err(|_| Error::InvalidBsonPayload)?).map_err(|_| Error::InvalidBsonPayload)?
        };
    ...
    fn encode_with_any_wrapper(&self, payload: Payload) -> Result<Vec<u8>, Error> {
        let (type_url, value_bytes) = match payload {
            ...
            bson::serialize_to_vec(&bson).map_err(|_| Error::InvalidBsonPayload)?
        )
        };
